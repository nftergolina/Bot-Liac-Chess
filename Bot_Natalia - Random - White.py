...
class MyBot(LiacBot):
    ...
    def on_move(self, state):
        ...
        move = []
        '''move.extend(self.translate_move(self.negamax(Node(state, [], state['who_moves']), state, 4, float('-inf'), float('inf'))))'''
        move.extend(self.translate_move(self.choose_random_move(state)))
    ...
    def choose_random_move(self, state):
        root = Node(state, [], state['who_moves'])
        root.generateChildren()
        moveList = []
        for child in root.children:
            moveList.append(child.move)

        return random.choice(moveList)
    
    def negamax(self, root, state, depth, a, b):
        root.generateChildren()
        bestValue = float('-inf')
        bestMove = []
        for child in root.children:
            for p in child.board.AllPieces:
                if p.team == root.board.BotTeam:
...
# MODELS ======================================================================
class Node(object):
    def __init__(self, previousState, move, t):
        self.children = []
        self.move = []
    ...
    def generateChildren(self):
        ...
        for p in self.board.AllPieces:
            if p.team == self.team:
                for m in p.possibleMoves:
                    self.children.append(Node(self.state, [p.position, m], t))
